...
def _set_runtime_properties(pid, ip, port):
    ctx.instance.runtime_properties.update({
        'pid': pid,
        'mongo_shard_host_{}'.format(ip): '{}:{}'.format(ip, port),
    })
...
def main():
    try:
        ...
        mongo_config_hosts = ctx.instance.runtime_properties['mongo_config_hosts']
        port = ctx.node.properties['port']
        ip = ctx.instance.host_ip

        process = subprocess.Popen([
                os.path.join(mongo_binaries_path, 'bin/mongos'),
                '--bind_ip', ip,
                '--port', str(port),
                '--configdb', mongo_config_hosts,
            ],
            stdout=open(os.path.join('/tmp', 'mongos.stdout'), 'w'),
            stderr=open(os.path.join('/tmp', 'mongos.stderr'), 'w'))

        _wait_for_server(port, process, 'MongoS')
        _set_runtime_properties(process.pid, ip, port)
        ctx.logger.info('Successfully started MongoS ({})'.format(process.pid))
    except:
